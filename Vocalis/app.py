...
class VocalisApp(tk.Tk):
    ...
    def _charger_voix(self) -> None:
        ...
        # Affiche dans le terminal exactement ce que l'interface reçoit de
        # TTSManager.load_voices(), pour vérifier si la voix Piper y figure
        # déjà à ce stade. À retirer une fois le problème confirmé résolu.
        print(
            f"[Vocalis][ui] _charger_voix() a reçu {len(voix)} voix : "
            + ", ".join(v.label for v in voix),
        ...
        # --- DIAGNOSTIC TEMPORAIRE ------------------------------------
        print(
            f"[Vocalis][ui] combo_voix['values'] = {self.combo_voix['values']}",
            file=sys.stderr, flush=True,
        )
        # ---------------------------------------------------------------
